...
module Hamiltonian
    ...
    SUBROUTINE estimate_num_neighbors(p_id, NNEIGH)
        ...
        INTEGER :: cell, num_cells, Nx,Ny,Nz, psize, tmp
        ...
        NNEIGH_per_site = 0
        Nx = 2*test_supercell(1)+1
        Ny = 2*test_supercell(2)+1
        Nz = 2*test_supercell(3)+1
        num_cells = Nx*Ny*Nz

        if(p_id.eq.0) THEN
            write(*,*) "Estimate number of neighbors (off-diagonals of Hamiltonian)..."
            write(*,*) "  --> use test supercell: ", Nx, Ny, Nz
        endif
        ...
        do cell=p_id,num_cells, psize  ! parallel go over all cells of the test supercell, each mpi process gets a separate cell
            ...
            ! This rather complicated calculation is used instead of three nested loops because it allows us to parallelize over all cells
            Sz1=mod(cell, Nz)
            Sy1=mod((cell-Sz1)/Nz, Ny)
            Sx1=mod(((cell-Sz1)/Nz-Sy1)/Ny, Nx)

            ! shift cells to make sure that the origin is in the middle of the supercell and we use all TIs
        ...
        tmp = MAXVAL(NNEIGH_per_site)  ! maximum value for this process

        ! get the maximum number of neighbors from all processes
        call MPI_ALLreduce(tmp, NNEIGH, 1, &
            &MPI_INTEGER, MPI_MAX, MPI_COMM_WORLD, ierr)  ! NNEIGH is output variable

        ! write(*,*) p_id, "NNEIGH = ", NNEIGH

        ! free memory
        deallocate(NNMAP)
        deallocate(TIsingle)
        deallocate(NNEIGH_per_site)
